...
const PropertyDetails = () => {
  ...
  useEffect(() => {
    const fetchPropertyData = async () => {
      try {
        const response = await axios.get(`${url}/property/${propertyId}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
          ...
          <Slider {...sliderSettings} className="property-slider">
            {property.images.map((image, index) => (
              <div key={index} className="slider-image">
                <img src={`https://raw.githubusercontent.com/prathameshchavan27/EasyHome-Property-Rental/refs/heads/master/Property-Rental-System-Backendsrc/main/resources/images/${image.imageUrl}`} alt={`Property ${index + 1}`} />
              </div>
            ))}
          </Slider>
        ) : (
